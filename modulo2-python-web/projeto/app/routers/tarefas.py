...
# Buscar tarefas pelo nome do  responsável
@router.get("/tarefas/responsavel/{nome}")
def tarefas_por_responsavel(nome: str):

# Filtrar as tarefas que pertencem ao resposável informado
      tarefas = [
          tarefa for tarefa in  tarefas_db
          if tarefa.responsavel.lower() == nome.lower
]

      # caso não encontre nenhuma tarefa
      if not tarefas:
          raise HTTPException(
              status_code=404,
...
#Alterar somente o status de uma tarefa
@router.patch("/tarefas/{tarefa_id}/status")
def alterar_status(tarefa_id: int, status: StatusEnum):

    #procura a tarefa pelo id
    for tarefa in tarefas_db:
        if tarefa.id == tarefa_id:

#não permite alterar uma tarefa que já foi cancelada
           if tarefa.status == StatusEnum.CANCELADA:
               raise HTTPException(status_code=400, detail="Não é possivel alterar o status de uma tarefa cancelada")
        ...
        # caso o ID não existia
        raise HTTPException(
            status_code=404, detail="Tarefa não encontrada"
        )

    # Buscar tarefa com prioridade critica
    @router.get("/tarefas/prioridade/critica")
    def tarefas_prioridade_critica():

        # Retorna somente tarefas de prioridade crítica
        # que ainda não forma concluidas ou canceladas
        return [
            tarefa for tarefa in tarefa_db
